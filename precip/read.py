    ...
    try:
        ...
        for line in lines:
            ...
            try:
                ...
                if data_value > 1000 or data_value < 0 or data_value == 999990:
...
def build_data_array(root_dir, start_date_str, end_date_str, station_codes, keyword='V13305_ZJ'):
    """
    构建时空数据数组（时间×站点）
...
def save_to_netcdf(output_path, data_array, date_list, station_dict, station_codes, 
    ...
    # 添加全局属性
    nc.description = 'Meteorological data from V13305_ZJ TXT files'
...
def main():
    ...
    KEYWORD = 'V13305_ZJ'                    # TXT文件名关键词
    BASE_NC_NAME = 'zj_prec'               # 基础NetCDF文件名
    OUTPUT_NC_PATH = f"{BASE_NC_NAME}_{START_DATE}-{END_DATE}.nc"    # 输出NetCDF文件路径
    VAR_NAME = 'prec'           # 气象变量名
    VAR_LONGNAME = 'Precipitation'  # 变量长名称
    VAR_UNITS = 'mm/day'                    # 变量单位（根据实际数据修改）
